...
const Faq = () => {
  ...
  return (
    <div className='px-4 lg:px-16 pt-28' id='faq'>
      <h2 className='max-w-96 h2 md:pl-8'>You've got questions, we've got answers.</h2>
      <p className='p mt-4 md:pl-8'>Find all your answers below</p>
      <div className='grid md:grid-cols-2 mt-16 gap-y-4 gap-x-16'>
      {faqs.map((faq, index) => (
        <FaqCard key={index} number={formatNumber(index + 1)} question={faq.question} answer={faq.answer} />
      ))}
      </div>
    </div>
  )
